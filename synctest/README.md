...
- [ ] Start a goroutine that sets a flag; call `synctest.Wait()` and assert the flag is set — verify you don't need `time.Sleep` to synchronise.
- [ ] Test a `context.WithTimeout` without real waiting: sleep to just before the deadline, assert no error; sleep past it, assert `DeadlineExceeded`.
- [ ] Test `context.AfterFunc`: verify the callback is not called before cancel and is called after — the example from the official docs, written from scratch.
